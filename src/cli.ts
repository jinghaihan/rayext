...
import { DEFAULT_EXTENSIONS_PATH, DEFAULT_RETRIES, MODE_ALIAS, MODE_CHOICES, NAME, VERSION } from './constants'
...
try {
  ...
  cli
    ...
    .option('--package <pkgs>', 'For monorepo support, specify the packages to install')
    ...
    .action(async (mode: RangeMode, options: Partial<CommandOptions>) => {
      ...
      await ensureExtensionsDir(DEFAULT_EXTENSIONS_PATH)
